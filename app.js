...
function collectBaseTasks(data){
  const out = [];
...
function renderTaskDetails(raw, template){
  ...
  kv('Spacing', raw.spacing_in || raw.final_spacing_in || '');
  kv('Row', raw.row_spacing_in || '');
    ...
    el.innerHTML = `
      <div class="row-top">
        <div>
          ...
          <div class="row-meta">${safeText(p.species || '')}${p.catalog_id ? ` • #${safeText(p.catalog_id)}`:''}</div>
        </div>
        <div class="row-actions">
...
function renderPlantDetail(plant, store){
  ...
  const planUI = `
    <div class="detail">
      ...
      <div class="kv"><div>Sowing method</div>
        <div>
          <select id="plan-method" class="input" style="width:100%; max-width:none">
            <option value="either">Either / decide later</option>
      ...
      <div class="kv"><div>Season</div>
        <div>
          <select id="plan-season" class="input" style="width:100%; max-width:none">
            <option value="both">Spring + Fall</option>
  ...
  const pestsBlock = `
      <h3>Pest & disease</h3>
      ${renderPests(plant.pest_disease)}
  `;
  ...
  const baseTasks = (plant.task_plan || []).map((t, idx) => normalizeTask(plantKey(plant), plant.name, plant.catalog_id, t, idx))
...
function renderPests(pd){
  ...
  const chip = (label) => {
    ...
    if (!src) return `<span class="badge">${safeText(label)}</span>`;
    return `<span class="pill"><img class="mini-icon" src="${safeText(src)}" alt="" /> ${safeText(label)}</span>`;
  };
...
function buildAllTasks(data, store){
  let tasks = collectBaseTasks(data);
